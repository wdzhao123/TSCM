...
class RS_dataset_ten(data.Dataset):
        ...
        for folder in os.listdir(dir):
            if self.part is not None:
                for images in os.listdir(dir + '/' + folder):
                    folder_index = reid_folder(folder)
...
class RS_dataset_trinity_ten(data.Dataset):
    def __init__(self, dir, transform_change=True, transform=dataTransform, image_size=256,
        ...
        if self.transform_change:
            self.transform = transform
        dir_list = [_ for _ in os.listdir(dir)]
        img = []
        for i in dir_list:
            ...
            if self.part is not None:
                for img_now_1 in os.listdir(dir_1):
                    ...
                    folder_index = reid_folder(i)
                    self.list_label.append(int(folder_index) - 1)
                    img = []
                all_len = len(self.list_img_now)
                part_len = all_len / self.part
                part_len = int(part_len)
    ...
    def __getitem__(self, item):
        ...
        img_trinity = torch.cat((img0, img1, img2), dim=0)
        label = self.list_label[item]
...
class RS_dataset_single_class_ten(data.Dataset):
    def __init__(self, dir, transform_change=False, transform=dataTransform, image_size=256):
        self.list_img = []
        self.list_label = []
        self.data_size = 0
        self.img_size = image_size
        ...
        for images in os.listdir(dir):
            ...
            folder_index = reid_folder(dir_index)
            folder_index = int(folder_index) - 1
            self.data_size += 1
            self.list_label.append(folder_index)
